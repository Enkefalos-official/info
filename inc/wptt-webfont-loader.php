...
if ( ! class_exists( 'Sports_Hub_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Sports_Hub_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their local sports_hub_counterparts.
		 */
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $sports_hub_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $sports_hub_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$sports_hub_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$sports_hub_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$sports_hub_font_family = trim( str_replace( array( "'", ';' ), '', $sports_hub_font_family ) );
					$sports_hub_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $sports_hub_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $sports_hub_font_family ] ) ) {
					$result[ $sports_hub_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$sports_hub_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$sports_hub_font_family_url = $this->get_absolute_path( $sports_hub_font_family_url );

					$result[ $sports_hub_font_family ][] = $sports_hub_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $sports_hub_font_family ] = array_flip( array_flip( $result[ $sports_hub_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Sports_Hub_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'wptt_get_websports_hub_font_url' ) ) {
	/**
